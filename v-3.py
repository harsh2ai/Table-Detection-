...
file=r'images\ms-9.jpg'
#file=r'/Users/YOURPATH/testcv.png'
img = cv2.imread(file,0)
img.shape#thresholding the image to a binary image
thresh,img_bin = cv2.threshold(img,130,255,cv2.THRESH_BINARY +cv2.THRESH_OTSU)#inverting the image 
img_bin = 255-img_bin
...
kernel_len = np.array(img).shape[1]//105# Defining a vertical kernel to detect all vertical lines of image 
...
#oth with 0.5. The aim is to get a clear tabular structure to detect each cell.

# Combine horizontal and vertical lines in a new third image, with both having same weight.
img_vh = cv2.addWeighted(vertical_lines, 0.7, horizontal_lines, 0.7, 0.0)#Eroding and thesholding the image
